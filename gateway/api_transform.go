...
func addOrUpdateApi(apiID string, r *http.Request) (interface{}, int) {
	...
	rdb = redis.NewClient(&redis.Options{
		Addr:         config.Global().RedisDBAppConfOptions.Host + ":" + strconv.Itoa(config.Global().RedisDBAppConfOptions.Port),
		Password:     config.Global().RedisDBAppConfOptions.Password,
		DB:           config.Global().RedisDBAppConfOptions.DB,
	...
	if Contains(api.Platforms, platform) {
		...
		if api.EnablePythonMiddleware {
			log.Info("Adding custom middleware folder for python %s", api.Name)
		...
		err = rdb.Set(api.APIID, apiJSON, 0).Err()
		if err != nil {
	...
	for _, jwtMeta := range jwtDefinitions.JWTDefinitions {
		...
		for {
			...
			ret := createJWTApiKey(tykConf, JWTAPIMap, jwtMeta.JWTPublicKeyPath, jwtMeta.JWTAPIKeyPath, host)
			count++
			if ret == true {
				break
			} else if count < 5 {
