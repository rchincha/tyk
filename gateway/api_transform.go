...
var (
	...
	TykRoot                      = "/local/nuchat/data/tyk-gateway/"
	TykCACert                    = "/local/nuchat/certs/cacerts.crt"
	TykServerCrt                 = "/local/nuchat/certs/server.crt"
	TykServerKey                 = "/local/nuchat/certs/server.key"
	TykUpstreamPem               = "/local/nuchat/certs/upstream.pem"
	SystemConfigFilePath         = "/local/nuchat/data/config/systemconfig.yaml"
	TykMiddlewareRoot            = "/local/nuchat/data/tyk-gateway/middleware/"
			...
			if Contains(api.Platforms, platform) {
				...
				if api.EnablePythonMiddleware {
					...
					if _, err := os.Stat(middlewareBundlePath); os.IsNotExist(err) {
						...
						log.Info("Added custom middleware folder for ", api.Name)
					}
				}

				if api.EnableGolangMiddleware {
					log.Info("Adding custom middleware folder for golang ", api.Name)
					temp["custom_middleware_bundle"] = TykMiddlewareBundleName
					...
					if _, err := os.Stat(middlewareBundlePath); os.IsNotExist(err) {
						...
						log.Info("Added golang middleware folder for ", api.Name)
					}
				}
				...
				log.Warn("Platform Missmatch .. skip adding: ", api.Name)
			}
		}
	}
...
func addOrDeleteJWTKey(e Event) error {
	...
	for _, jwtMeta := range jwtDefinitions.JWTDefinitions {
		...
		for {
			...
			ret := createOrDeleteJWTApiKey(tykConf, JWTAPIMap, jwtMeta.JWTPublicKeyPath, jwtMeta.JWTAPIKeyPath, "localhost", e)
			count++
			if ret == true {
				break
			} else if count < 3 {
...
func createOrDeleteJWTApiKey(tykConf map[string]interface{},
	...
	if (e == ADD && len(jwtAPIMap) > 0) || (e == DELETE && len(jwtAPIMap) > 0) {
		...
		req.Header.Add("X-Tyk-Authorization", tykConf["secret"].(string))
		log.Info("Deleting JWT Token: %s", string(JWTApiKey))
		resp, err := client.Do(req)
		if err != nil {
